...
/**
 * Excel超过两万条用poi读取会出现out of memory异常
 ...
 * @version 2014-9-2
 */
public abstract class BigExcelReader extends DefaultHandler {
    ...
    //标签开始的时候回调 eg:<c> <v>
    //一般底层标签是<c> <v> </v> </c>  c代表一个单元格，v代表该单元格的值 c标签内还会有单元格的其他属性即其他标签
    //比如<c> <f> </f> <v> </v> </c>  f标签代表该单元格的函数
    //一行会有<row>标签
    @Override
...
//      System.out.println("endElement: " + localName + ", " + name);
        ...
        if((name.equals("v")||name.equals("c"))&&isNullCell){
            rowList.add(curCol, "");
            curCol++;
            isNullCell=false;
        }
    ...
    /**
     * 获取行数据回调
     * @param sheetIndex 目前读取的sheet索引
     * @param curRow  当前行数
     * @param rowList 一行中每个cell的数据都是list的一个元素
     */
